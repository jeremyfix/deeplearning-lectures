#python3 train_one_object.py --dataset_dir /opt/Datasets/Pascal-VOC2012/ --num_workers 6 --resize_image shrink  --target_mode largest_bbox --base_model resnet18 --use_gpu
...
if __name__ == '__main__':
    ...
    parser.add_argument(
            '--model',
            choices=['resnet18', 'resnet34','resnet50','resnet152','densenet121','squeezenet1_1','mobilenetv2'],
    ...
    if args.debug:
        utils.extract_save_features(train_loader, model, device, "train.pt", N=1)
        utils.extract_save_features(valid_loader, model, device, "valid.pt", N=1)
    else:
        utils.extract_save_features(train_loader, model, device, "train.pt")
        utils.extract_save_features(valid_loader, model, device, "valid.pt")
