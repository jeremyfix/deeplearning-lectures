...
class LinearModel(nn.Module):
    ...
    def decode(self, inputs: PackedSequence) -> List[Tuple[float, str]]:
        ...
        return greedy_decode(outputs, self.charmap)
...
def ex_pack():
    ...
    tensors = [
        torch.randn(random.randint(1, max_length), n_mels) for i in range(batch_size)
    ]

    # To be packed, the tensors need to be sorted by
    # decreasing length
    tensors = sorted(tensors, key=lambda tensor: tensor.shape[0], reverse=True)
    ...
    packed_data = pack_padded_sequence(tensors, lengths=lengths, batch_first=True)

    # Latter, we can unpack the sequence
    unpacked_data, lens_data = pad_packed_sequence(packed_data, batch_first=True)
