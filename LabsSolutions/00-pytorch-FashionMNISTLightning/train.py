...
class LitModel(pl.LightningModule):
    def __init__(
        self, example_input_array_size, weight_decay, model: nn.Module = None
    ) -> None:
        super().__init__()
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(
            self.model.parameters(), lr=0.001, weight_decay=self.weight_decay
        )
...
def main(args):
    ...
    lmodel = LitModel((batch_size,) + img_size, args["weight_decay"], model)
    ...
    metrics = trainer.test(dataloaders=test_loader, ckpt_path="best")
