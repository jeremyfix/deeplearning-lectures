...
def train(config):
    ...
    rootdir = config["data"]["rootdir"]
    nthreads = config["data"].get("nthreads", 6)
    batch_size = config["data"]["batch_size"]

    logdir = config["logging"]["logdir"]

    base_lr = config["optim"]["base_lr"]
    wdecay = config["optim"]["wdecay"]
    lblsmooth = config["optim"]["lblsmooth"]

    nepochs = config["nepochs"]

    dnoise = config["model"]["dnoise"]
    discriminator_base_c = config["model"]["discriminator_base_c"]
    generator_base_c = config["model"]["generator_base_c"]
    ...
    logdir = generate_unique_logpath(logdir, "gan")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        logging.error(f" - {sys.argv[0]} generate generator.onnx")
        sys.exit(-1)
