...
def make_linear(
    input_shape, num_classes, normalization=None, use_dropout=False, use_L2=False
):
...
def make_fc(
    ...
    if use_L2:
        x = Dense(
            nhidden1, kernel_regularizer=regularizers.l2(L2_reg), activation="relu"
        )(x)
    else:
    ...
    if use_L2:
        x = Dense(
            nhidden2, kernel_regularizer=regularizers.l2(L2_reg), activation="relu"
        )(x)
    else:
...
def conv_relu_max(nfilters, x):
    x = Conv2D(filters=nfilters, kernel_size=5, padding="same")(x)
    x = Activation("relu")(x)
    x = MaxPooling2D(pool_size=2, padding="same")(x)
    return x


def make_vanilla_cnn(
    input_shape, num_classes, normalization=None, use_dropout=False, use_L2=False
):
...
def fancyBlock(nfilters, x, last):
    x = Conv2D(filters=nfilters, kernel_size=3, padding="same")(x)
    x = Activation("relu")(x)
...
def build_network(model, input_shape, num_classes, normalization, use_dropout, use_L2):
    ...
    if normalization is not None:
        # print(normalization)
        lambda_normalization = lambda xi: Lambda(
            lambda image, mu, std: (image - mu) / std,
            arguments={"mu": normalization[0], "std": normalization[1]},
        )(xi)
    return builders[model](
        input_shape, num_classes, lambda_normalization, use_dropout, use_L2
    )
