...
This loss is to be minimised with respect to the parameters of the generator $\theta_g$. The original paper considered fully connected architectures for the critic and the generator but the later work of Deep Convolutional GAN (DCGAN, [@Radford2016]) proposed to use convolutional networks for the discriminator and the generator. Our aim is to generate fake digits. The image below show you some examples of digits generated by the generator you will train in this labwork.

![Fake digits. Nobody has ever written these digits.](data/03-pytorch-gan/generated-mnist.png){width=50%}
...
Below is an example of digits generated during a successful training with a noise vector defined once for all before training (i.e. we always plot the generated image associated with the same random inputs).
