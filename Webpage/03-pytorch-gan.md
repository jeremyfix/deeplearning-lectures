...
You may recognize the usual binary cross entropy loss where the labels of the $m$ real data is set to $y^x_i=1$ and the labels of the $m$ fake data is set to $y^z_i=0$. This loss is to be minimized with respect to the parameters of the critic $\theta_d$.
...
The original paper considered fully connected architectures for the critic and the generator but the later work of Deep Convolutional GAN (DCGAN, [@Radford2016]) proposed to use convolutional networks for the discriminator and the generator. Our aim is to generate fake digits. The image below show you some examples of digits generated by the generator you will train in this labwork.

![Fake digits. Nobody has ever written these digits.](data/03-gan/generated-mnist.png){width=50%}
...
**Exercice** Implement the critic in the `data.py` script. You have to define the neural network in the constructor Discriminator class and implement the forward method.
