...
$$
\forall i \in [0, d-1], E(x)_{[|i 2 L:(i+1)2L|]} = [\cos(2^0 \pi x_i), \cos(2^1 \pi x_i), \cdots \cos(2^{L-1} \pi x_i), \sin(2^0 \pi x_i), \sin(2^1 \pi x_i), \cdots \sin(2^{L-1} \pi x_i)]
$$

The above formula clearly states that we have a slice of the embedding for each component of our point $x$. So far we discussed the case $d=2$ for an image but the positional encoding can be written in a very generic way, for arbitrary input dimension $d$.
...
The expected output is given below. On this image, the encoding of $x=0.425$ is shown with the black markers.
