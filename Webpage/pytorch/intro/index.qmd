...
The final category of regularization techniques we will consider is data augmentation. Data augmentation are transforms of your inputs for which you can predict the impact on the target. For this, you can use dedicated libraries such as [albumentations](https://albumentations.ai/) but we will here stick to the pytorch ecosystem with the [torchvision v2 transforms](https://pytorch.org/vision/main/auto_examples/transforms/plot_transforms_getting_started.html). 

For a problem such as classifying fashion furnitures, we can consider transforms that leave invariant the class such as :

- rotations
- translations
- symmetries
- zoom
- luminosity, contrast, .. alterations

These augmentations are performed *on the fly*, no need to precompute a transformed dataset. The data augmentation transforms are added in the data pipeline, hence in the `data.py` submodule. For now, you only have : 
...
You may also want to try augmentation poliices such as AutoAugment, RandAugment, etc.. , see [Auto-Augmentation](https://pytorch.org/vision/main/transforms.html#auto-augmentation).
:::

::: {.callout-tip}
**Experiment** training with data augmentation. For your experiments, you need a model with sufficient capacity to be able to overfit, otherwise, you will not see the impact of data augmentation. For example, a multilayer feedforward network with $5$ hidden layers of size $128$.
:::

::: {.callout-note}
I advise you to calibrate your augmentation pipeline, writting a function in `data.py` that will display augmented samples. That's a very first approach to define this pipeline. At the end of the day, it's your real risk that will gear you toward the appropriate augmentations.
:::
...
Finally, we are going to implement a convolutional neural network. For that, you need to complete the implementation of the `FancyCNN` class in the `models/cnn_models.py` subdmodule. We will then experiment writting a class deriving from the torch.nn.Module class :
...
After every $2$ subblocks, we add a downsampling subblock composed of :
...
You should end up with a model reaching around $93\%$ of test accuracy, and a test cross entropy loss around $0.20$ with a FancyCNN of $570K$ parameters, RandomHFlip and RandomRotation($-10$ deg, $10$ deg), training for $$20$ epochs. The training accuracy reached $95\%$, with a loss of $0.14$.
