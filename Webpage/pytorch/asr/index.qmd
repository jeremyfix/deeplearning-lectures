...
After completion, write a little piece for testing which should : 1) access a minibatch of one of your dataloader, 2) unpack both the spectrograms and transcripts 3) plot the spectrograms associated with their transcripts. For the plot part, you can make use of the `plot_spectro` function (in `data/__init__.py`).
...
**Exercice** Define in the constructor the *cnn* attribute. Test your implementation by calling the `test_model_cnn` function of the `models` submodule. As usual with :

```bash
(venv) mymachine:~:mylogin$ python -m asrlab.models
```
...
**Exercice** Define in the constructor the *rnn* attribute. Test your implementation by calling `test_model_rnn` function of the `models` submodule :
...
**Exercice** Define in the constructor the *char_lin* attribute. Test your implementation by calling `test_model_out` function of the `models` submodule :
...
All right, we now move on the `main.py` script. This script will allow you to train and test your networks. You can open, read and run this script. It provides : 
...
- a main function handling the parameters, at the very of the script.
...
**Exercice** Instantiate the model and define the optimizer in the `train` function of the `main.py` script. For the parameters of the optimizer and model, see the dedicated part in the `config-sample.yaml` file :
...
model:
  ...
  params:
    ...
    cell_type: GRU
    dropout: 0.5
    n_mels: 80 # TODO: must be the same as data["nmels"]
...
In order to ensure you do not have bugs in your implementation, a sanity check is to check you can overfit a single minibatch of your training set. If you are unable to fit a single minibatch, you are guaranteed to have an issue in your code. To just load one single minibatch, you can limit the number of samples considered for training, by specifying the `num_samples` parameter in the `data` section of the yaml configuration.

```bash
(venv) mymachine:~:mylogin$ python -m asrlab.main train config.yaml
```

with the following part in your `config.yaml` :
...
```bash
(venv) mymachine:~:mylogin$ python -m asrlab.main train config-overfit.yaml
```
...
SpecAugment is already implemented in the `WaveformProcessor` object in the `data/waveform.py` script. Have a look to the transform_augment attribute of WaveformProcessor. To use it, simply add the `train_augment: True` argument in the `data` section of the yaml script you use for running a simulation. This data section is the one transfered to the `get_dataloaders` function which will then request the `WaveformProcessor` to perform both frequency and time masking.
...
You can run a training with a reasonnable default configuration : 

```bash
(venv) mymachine:~:mylogin$ python -m asrlab.main train config-sample.yaml
```

This configuration adds a `scheduler` on the learning rate. It was disabled for the previous experiments, when overfitting a single minibatch, as it was decreasing the learning rate too quickly. 

In the provided configuration, you may also notice the Dropout was raised from $0.0$ to $0.5$. 
...
7.25      ¶l'aversure moderne est une reconstitution a partir de fradment fran❌ais espagnol les portugais.¦
```

The true transcript is `La version moderne est une reconstitution à partir de fragments français, espagnol et portugais.`.
